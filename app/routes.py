        ...
        if os.path.exists(cache_filepath):
            ...
            async with scrape_lock:
                async with aiofiles.open(cache_filepath, 'r', encoding='utf-8') as f:
                    cached_url_mapping = json.loads( f.read())
...
async def screenshot_status(task_id):
    ...
    if not task_info:
        ...
        return  render_template('error.html', message='Screenshot Task nicht gefunden.'), 404
    ...
    if task_info['status'] == 'completed':
        ...
        return  render_template('screenshot_load.html', task_id=task_id)
...
async def screenshot_result(task_id):
    ...
    try:
        ...
        if not os.path.exists(zip_filepath):
            ...
            return  render_template('error.html', message='Screenshots nicht verfügbar.'), 404

        return  render_template('screenshot_result.html', zip_filename=zip_filename, task_id=task_id)

    except Exception as e:
        logger.exception(f"Fehler beim Anzeigen der Screenshot-Ergebnisse für Task {task_id}:")
        return  render_template('error.html', message='Fehler beim Anzeigen der Screenshot-Ergebnisse.'), 500
...
async def download_screenshots(task_id):
    ...
    try:
        ...
        if status != 'completed':
            ...
            return  render_template('error.html', message='Screenshots sind noch nicht verfügbar.'), 404
        ...
        return  render_template('error.html', message='Fehler beim Herunterladen der Screenshots.'), 500
...
async def combined_image(task_id):
    try:
        ...
        if not task_info:
            ...
            return  render_template('error.html', message='Kombiniertes Bild nicht verfügbar.'), 404

        status = task_info.get('status')
        if status != 'completed':
            logger.warning(f"Kombiniertes Bild für Task {task_id} ist noch {status}.")
            return  render_template('error.html', message='Kombiniertes Bild ist noch nicht verfügbar.'), 404
        ...
        if not combined_image_path:
            ...
            return  render_template('error.html', message='Kombiniertes Bildpfad nicht gefunden.'), 404
        ...
        return  render_template('error.html', message='Fehler beim Anzeigen des kombinierten Bildes.'), 500
